...
public class AttributeConstraintsLibraryLoader {
	...
	public Collection<AttributeConstraintLibrary> run(final ResourceSet resourceSet) {

		final Collection<AttributeConstraintLibrary> attributeConstraintLibraries = new ArrayList<>();
		for (final Resource resource : resourceSet.getResources()) {
			if (!resource.getContents().isEmpty()) {
				...
				if (firstObject instanceof EditorGTFile) {
					...
					attributeConstraintLibraries.add(loadAttributeConstraintsLibrary(editorGtFile));
				}
			}
		}
